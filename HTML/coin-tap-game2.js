...
(function () {
	...
	function spawnCoin () {
		...
		// Load sprite sheet
		coinImg.src = "bone_spritesheet.png";  
	}
	function spawnchoco () {
		...
		chocoIndex = choco.length;
		
		// Create sprite
		choco[chocoIndex] = sprite({
			context: canvas.getContext("2d"),
			width: 1000,
			height: 100,
			image: chocoImg,
		...
		choco[chocoIndex].x = Math.random() * (canvas.width - choco[chocoIndex].getFrameWidth() * choco[chocoIndex].scaleRatio);
		choco[chocoIndex].y = Math.random() * (canvas.height - choco[chocoIndex].height * choco[chocoIndex].scaleRatio);
		
		choco[chocoIndex].scaleRatio = Math.random() * 0.5 + 0.5;
		
		// Load sprite sheet
		chocoImg.src = "chocolate_spritesheet.png";  
	}
	...
	function tap (e) {
		...
		for (j = 0; j < choco.length; j += 1) {
			...
			// Distance between tap and coin
			dist = distance({
				x: (choco[j].x + choco[j].getFrameWidth() / 2 * choco[j].scaleRatio),
				y: (choco[j].y + choco[j].getFrameWidth() / 2 * choco[j].scaleRatio)
			}, {
				x: loc.x,
				y: loc.y
			});
			...
			// Check for tap collision with coin		
			if (dist < choco[j].getFrameWidth() / 2 * choco[j].scaleRatio) {
				chocoToDestroy.push(choco[j]);
			}
		}
		
		// Destroy tapped chocos
		for (j = 0; j < chocoToDestroy.length; j += 1) {
		
			score += parseInt(chocoToDestroy[j].scaleRatio * 10, 10);
			destroychoco(chocoToDestroy[i]);	
			setTimeout(spawnchoco, 1000);	
		}
		
		if (chocoToDestroy.length) {
			document.getElementById("score").innerHTML = score;
		}
	}
